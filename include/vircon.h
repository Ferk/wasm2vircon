...
static inline void print_at(int x, int y, const char *text) { vircon__print_at(x, y, text); }
...
static int vircon__print_digits(int x, int y, unsigned value) {
  ...
  if (quotient != 0)
    x = vircon__print_digits(x, y, quotient);
  digit_text[0] = (char)('0' + value - quotient * 10u);
  print_at(x, y, digit_text);
  return x + 10;
}
static inline void print_uint_at(int x, int y, unsigned value) { (void)vircon__print_digits(x, y, value); }
...
static inline void get_drawing_point(int *x, int *y) {
  *x = vircon__gpu_get_drawing_point_x();
  *y = vircon__gpu_get_drawing_point_y();
}
...
static inline void get_drawing_scale(float *x, float *y) {
  *x = vircon__gpu_get_drawing_scale_x();
  *y = vircon__gpu_get_drawing_scale_y();
}
...
static inline void draw_region_zoomed_at(int x, int y) {
  set_drawing_point(x, y);
  vircon__gpu_draw_region_zoomed();
}
...
static inline void draw_region_rotated_at(int x, int y) {
  set_drawing_point(x, y);
  vircon__gpu_draw_region_rotated();
}
...
static inline void draw_region_rotozoomed_at(int x, int y) {
  set_drawing_point(x, y);
  vircon__gpu_draw_region_rotozoomed();
}

/* Function-like macros preserve the official six/four-argument call surface
 * and apply each component GPU write in order. Each input is evaluated once
 * into a block-local i32 before it is applied to GPU state. */
...
static inline void gamepad_direction(int *x, int *y) {
  *x = gamepad_direction_x();
  *y = gamepad_direction_y();
}
...
static inline void sleep(int frames) {
  ...
  while (get_frame_counter() < final_frame)
    end_frame();
}
...
static inline void play_sound_in_channel(int sound, int channel) {
  assign_channel_sound(channel, sound);
...
static inline int card_read_word(int word_index) { return vircon__memcard_read_word(word_index); }
static inline void card_write_word(int word_index, int value) { vircon__memcard_write_word(word_index, value); }
...
static inline void card_write_data(const void *source, int card_word_offset, int word_count) {
  ...
  for (index = 0; index < word_count; ++index) {
    ...
    unsigned word = (unsigned)bytes[byte_index] | ((unsigned)bytes[byte_index + 1] << 8) |
                    ((unsigned)bytes[byte_index + 2] << 16) | ((unsigned)bytes[byte_index + 3] << 24);
...
static inline int isdigit(int character) { return character >= '0' && character <= '9'; }
...
static inline int isascii(int character) { return character >= 0 && character <= 127; }
static inline int isalphanum(int character) { return isdigit(character) || isalpha(character); }
/* Includes the Windows-1252 accented Latin ranges used by the BIOS font. */
static inline int islower(int character) {
  return (character >= 'a' && character <= 'z') || (character >= 224 && character <= 254 && character != 247);
}
static inline int isupper(int character) {
  return (character >= 'A' && character <= 'Z') || (character >= 192 && character <= 222 && character != 215);
}
static inline int isspace(int character) {
  return character == ' ' || character == '\n' || character == '\r' || character == '\t';
}
static inline int tolower(int character) { return isupper(character) ? character + 32 : character; }
